    ...
    def apply_updates(self, delta, td_clip=5.0):
        ...
        if self.training_mode:
            ...
            if hasattr(self, '_trace_update_count') and self._trace_update_count % 100 == 0:
                actor_trace_norm = torch.norm(self.e_a)
                critic_trace_norm = torch.norm(self.e_c)
                actor_update_norm = torch.norm(self.lr_a * delta * self.e_a)
                critic_update_norm = torch.norm(self.lr_c * delta * self.e_c)
                
                print(f"  TD error (δ): {delta.item():.4f}")
...
def train(headless=True, num_episodes=500, args=None):
    ...
    if args:
        ...
        if args.load_checkpoint:
            ...
            # If disturb_weights is set, add noise to the loaded weights
            if args.disturb_weights is not None and not args.load_hparams_only:
                noise_std = args.disturb_weights
                print(f"Disturbing weights with Gaussian noise (std={noise_std})")
                
                # Add noise to actor and critic weights
                ac.W_a += torch.randn_like(ac.W_a) * noise_std
                ac.W_c += torch.randn_like(ac.W_c) * noise_std
                
                # Optionally disturb log_std as well (small amount)
                ac.log_std += torch.randn_like(ac.log_std) * noise_std * 0.1
                
                # Reset traces after disturbance
                ac.e_a = torch.zeros_like(ac.W_a)
                ac.e_c = torch.zeros_like(ac.W_c)
                ...
                print(f"Weights disturbed. Actor norm: {torch.norm(ac.W_a).item():.4f}, Critic norm: {torch.norm(ac.W_c).item():.4f}")
            ...
            ac = ActorCritic(obs_dim=obs_dim, act_dim=act_dim, device=device,
                            ...
                            noise_std=args.noise_std, grad_clip_norm=args.grad_clip_norm)
            gamma = args.gamma
            reward_scale = args.reward_scale
            td_clip = args.td_clip
...
if __name__ == "__main__":
    ...
    parser.add_argument('--disturb-weights', type=float, default=None,
                        help='Add Gaussian noise with this std dev to loaded weights (use with --load-checkpoint)')
